...
\begin{abstract}
\cn{Abstract结构：一句背景→一句问题→一句方法→5点核心发现→一句建议。所有数字均来自results/main_results.json，审稿人可逐条验证。}
...
\cn{背景痛点：以near-perfect accuracy先承认领域进展，然后用subtle but critical flaw转折，指出滑动窗口随机切分的根本问题——相邻窗口共享大量信号内容，导致分类器在做recording identification而非故障诊断。这种先扬后抑的写法能迅速引起审稿人对泄漏问题的重视。}
...
\cn{问题定位与研究空白：承认数据泄漏已为人知，但指出其与数据增强的交互未被系统研究——这是本文的核心贡献定位。用in principle和but的对立句式制造张力。}
...
\cn{假设陈述：直接列出三个假设，结构清晰，为后文的结果讨论做铺垫。务必在Discussion中逐一回应每个假设。}
...
\cn{数据集与训练细节：在引言末尾交代数据集规模和训练配置，为可重复性提供早期保证。不是Method的完整细节，而是让读者在引言阶段就能判断实验规模是否可信。}
...
\cn{CWRU基准：以数据集本身开篇，立即建立文献合法性。引用Smith \& Randall的原版基准论文作为权威来源。}
...
\cn{增强与故障诊断：介绍SpecAugment作为代表性频域增强方法，同时指出物理含义被忽视——这是本文区别于纯计算机视觉方法的关键。}
...
\cn{时序泄漏：引用Bergmeir \& Benítez（2012）作为时序交叉验证的奠基性文献，直接将图像领域的泄漏讨论迁移到振动信号。}
...
\cn{物理约束增强：指出前人做了物理约束增强但未测试其与性能的关系——这是H3的文献支撑。}
...
\cn{数据集细节：逐类列出录制文件数量，确保审稿人能精确还原数据规模。40个文件、11832个窗口是关键数字。}
...
\cn{窗口分割参数：1024点窗口约85ms（在12kHz采样率下），50%重叠是领域常见做法。声明窗口分布顺便暗示数据不平衡程度——Normal仅占8%，故障类各占约30%。}
...
\cn{泄漏机制：点明重叠导致的泄漏路径——512点共享内容即窗口间50%的信号完全一致。}
...
\cn{泄漏控制方案：说明recording-level split的操作定义，强调按类别分层和60/20/20的比例，满足机器学习的基本训练/验证/测试规范。}
...
\cn{引用前向：提前引用M1机理验证结果，在数据章节就建立证据链，避免审稿人等待太久才看到泄漏的实证。}
...
\cn{安全实践清单：列出归一化、增强时机、参数固定三项关键实践，证明实验未引入额外泄漏。这是manual明确要求的泄漏审计内容。}
...
\cn{实验设计因子列表：四个因子完整展开。注意因子顺序暗含研究框架：模型→协议→增强→种子，从固定架构到随机化，逐层嵌套。}
...
\cn{消融实验设计：三种分组方式对应三种域偏移维度。这是方向一要求的ablation，也是论文的加分项。}
...
\cn{机理验证总述：引用课程手册作为方法论依据，强调在分类之前完成验证。这是manual要求的证据层级。}
...
\cn{M1邻窗自相关：这里的表述比较谨慎（specific correlations exist而非high correlation），因为实际数值并不高。诚实报告比夸大效果更好。}
...
\cn{M5故障频带能量审计：所有增强通过安全检验的事实很重要——它说明H3的零结果不是因为增强破坏了物理特征。要区分"通过安全检查"与"有助于分类"。}
...
\cn{M2特征多样性：M2的主要结论是所有增强都增加多样性，这为H3的零发现提供了一个解释——增强通过增加多样性而非保留物理特征来帮助（或不帮助）分类。}
...
\cn{实验设置：使用itemize列表统一收敛性报告，方便审稿人快速定位超参数。收敛分析的结果（中位数epoch=9）是对50epoch选择的事后辩护。}
\begin{itemize}[nosep]
...
\cn{Table 1解读：写论文最怕的就是表格数据没人读，所以一定要在正文里解读关键数字。这里挑出gap和d值，引导审稿人关注最重要的发现。}
...
\cn{2D-CNN的负面结果：这个发现很重要且反直觉——大多数增强居然有害。明确列出哪些增强有用、哪些有害，不可含糊其辞。}
...
\cn{Per-class结果解读：不要避免指出最差类（IR和B）。诚实报告每个类的短板比只报平均指标更有说服力。}
...
\cn{Gap-recovery解读：Δrec是本文的核心指标，必须用一句话定义清楚。negative Δrec意味着增强后比不增强还差——这很反直觉，要引起审稿人注意。}
...
\cn{消融解读：fault\_size >> recording > load的结论是本文最强发现之一。cross-severity generalization这个术语是领域通用语，使用它能增加论文的专业性。}
...
\cn{H3被证伪：诚实报告null result需要框架——这里将negative Spearman解释为noise-as-regularizer假说，把不显著的发现转化为建设性讨论。}
...
\cn{单增强vs组合增强：这个消融证明stacking不总是有益的，与常见做法相反。用具体数字对比而非只陈述结论。}
...
\cn{分组变量重要性小结：避免重复前面的结果，用简洁的总结句引用前面的消融章节。near-complete failure这个表述是合理且有力的学术判断。}
...
\cn{H1讨论：先确认成立再指出局限。2D弱于1D的现象可以引发关于spectrogram信息损失的讨论。}
...
\cn{H2讨论：用Cohen's d的定量语言确保论点的统计严谨性。proper evaluation protocol一词强调方法论意义。}
...
\cn{H3讨论：honest null result是对无法拒绝零假设的诚实措辞。将发现归因于sample diversity而非feature corruption，为增强选择提供新视角。}
...
\cn{3-seed不足的教训：从初版3-seed实验的错误中提炼方法论建议。叙述视角从what we found转向what we learned，更有教学意义。}
...
\cn{跨严重度泛化：将消融发现的含义提升到领域级别。这句话是整篇论文的hook——把泄漏讨论重新框架化为一个更根本问题的子集。}
...
\cn{局限性列表：5条对应5类风险——数据集、传感器、参数、架构、训练轮次。每条的表述模式：问题描述+潜在影响或替代方案。不要只罗列不解释。}
...
\cn{结论段：遵循标准结构——重申核心发现→量化效果→指出最限制因素（跨严重度泛化）→方法论建议→开放数据承诺。不需要新信息，只需要强有力地复述Results+Discussion。}
...
\cn{可复现性声明：包含代码位置、种子列表、日志位置、再生指令四要素。凡是无法仅凭论文正文复现的信息，都应在此披露。}
...
\cn{AI使用披露：按功能分类——代码实现、调试、起草。结尾的universal denial是防止M3（幻觉结果）的基础防护。}
